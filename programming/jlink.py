...
class JLink(object):
	def make_uicr_bin(self, bootloader_addr):
		...
		try:
			...
			if os.write(fd, bytes(uicr)) != 4:
				utils.errorHandler("Write error, wrote " + str(bytesWritten) + " expected 4", self.cleanup)
	...
	def make_script(self, device, softdevice, bootloader, 
		...
		datapage_addr, readback_protect):
		...
		if readback_protect:
			jlink_file.write('w4 4001e504 1\n')
			jlink_file.write('loadbin protect.bin 0x10001004\n')
