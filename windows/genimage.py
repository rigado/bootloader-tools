...
class RigDfuGen(object):
    ...
    def find_bl(self):
        ...
        # 0x30000 and 0x3f800
        (minaddr, maxaddr) = self.data.extents(0x30000, 0x3f800, 0x400)
        if minaddr is None:
            return False
        for addr in range(minaddr, maxaddr, 0x400):
            if self.valid_code(addr, maxaddr):
                break
        else:
            return False
        # Address of BL is the extent of valid code we found
        self.bl_addr = self.data.extents(addr, 0x3f800)
        return True
    ...
    def find_app(self):
        ...
        # Find valid code at the beginning of a page somewhere between
        # 0x10000 and 0x3f000.
        (minaddr, maxaddr) = (0x10000, 0x3f000)
        ...
        (minaddr, maxaddr) = self.data.extents(minaddr, maxaddr, 0x400)
        if minaddr is None:
            return False
        for addr in range(minaddr, maxaddr, 0x400):
            if self.valid_code(addr, maxaddr):
